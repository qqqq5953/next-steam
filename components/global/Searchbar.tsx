...
export default function Searchbar() {
  ...
  useEffect(() => {
    ...
    async function searchForGames(keyword: string) {
      const res = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/games/search?query=${keyword.trim()}`)
    ...
    async function searchForCollections(keyword: string) {
      const res = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/collections/search?query=${keyword.trim()}`)
    ...
    async function searchForCreators(keyword: string) {
      const res = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/creators/search?query=${keyword.trim()}`)
